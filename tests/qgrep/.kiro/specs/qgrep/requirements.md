...
- 3.6 フィルタ適用後の検索対象ファイルが0件ならば、`qgrep CLI` は一致結果を出力してはならない。
...
- 5.7 一致行の先行に `N` 行未満しか存在しないならば、`qgrep CLI` は存在する行のみを先行コンテキストとして出力しなければならない。
- 5.8 一致行の後続に `N` 行未満しか存在しないならば、`qgrep CLI` は存在する行のみを後続コンテキストとして出力しなければならない。
...
- 7.8 JSON 出力オプションを指定した場合、`qgrep CLI` は出力する各行を単一の妥当な JSON オブジェクトとして出力しなければならない。
