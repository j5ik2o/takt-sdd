...
function makeCurrentSurfaceFixture() {
  const root = makeFixture();
  const repoRoot = join(import.meta.dirname, "..");
  writeFixtureFile(root, ".takt/config.yaml", "language: ja\n");
...
test("staged Kiro wrapper resolves configured language workflow path", () => {
  ...
  assert.equal(resolveWorkflowPath(root, "kiro-impl"), join(root, "builtins", "ja", "workflows", "kiro-impl.yaml"));
...
test("staged Kiro wrapper resolves configured language from config.yml fallback", () => {
  ...
  assert.equal(resolveWorkflowPath(root, "kiro-impl"), join(root, "builtins", "en", "workflows", "kiro-impl.yaml"));
});

test("staged Kiro wrapper does not implicitly fall back to module package builtins for arbitrary root", () => {
  const root = makeFixture();
  writeFixtureFile(root, ".takt/config.yaml", "language: en\n");

  assert.equal(resolveWorkflowPath(root, "kiro-impl"), undefined);
});

test("staged Kiro wrapper can fall back to explicit package root builtins", () => {
  const root = makeFixture();
  const repoRoot = join(import.meta.dirname, "..");
  writeFixtureFile(root, ".takt/config.yaml", "language: en\n");

  assert.equal(
    resolveWorkflowPath(root, "kiro-impl", repoRoot),
    join(repoRoot, "builtins", "en", "workflows", "kiro-impl.yaml"),
  );
});
...
test("validator rejects reviewer adapters without Kiro Skill Source instructions", () => {
  const root = makeCurrentSurfaceFixture();
  const facetPath = join(root, "builtins", "en", "facets", "instructions", "kiro-review-architecture-task.md");
  ...
  writeFixtureFile(root, "builtins/en/facets/instructions/kiro-review-architecture-task.md", facet);
...
test("validator reports missing kiro-impl workflow and package wiring", () => {
  ...
  mkdirSync(join(root, "builtins", "ja", "workflows"), { recursive: true });
...
test("validator rejects standalone adapter workflows", () => {
  ...
  for (const lang of ["en", "ja"]) {
    writeFixtureFile(root, `builtins/${lang}/workflows/kiro-review.yaml`, "name: kiro-review\nsteps: []\n");
  }
...
test("validator rejects custom retry or loop health source of truth in workflow", () => {
  ...
  const repoRoot = join(import.meta.dirname, "..");
  const workflow = readFileSync(join(repoRoot, "builtins", "ja", "workflows", "kiro-impl.yaml"), "utf8")
    .concat("\n# maxAttempts: 3\n");
  for (const lang of ["en", "ja"]) {
    writeFixtureFile(root, `builtins/${lang}/workflows/kiro-impl.yaml`, workflow);
  }
...
test("validator rejects completion-before-checkbox gate drift", () => {
  const root = makeCurrentSurfaceFixture();
  const workflowPath = join(root, "builtins", "en", "workflows", "kiro-impl.yaml");
  const workflow = readFileSync(workflowPath, "utf8").replace(
    "STATUS VERIFIED and safe_to_update_progress true",
    "STATUS VERIFIED",
  );
  writeFixtureFile(root, "builtins/en/workflows/kiro-impl.yaml", workflow);
...
test("validator rejects readiness routing that skips ready-for-implementation", () => {
  const root = makeCurrentSurfaceFixture();
  const workflowPath = join(root, "builtins", "ja", "workflows", "kiro-impl.yaml");
  const workflow = readFileSync(workflowPath, "utf8").replace(
  ...
  writeFixtureFile(root, "builtins/ja/workflows/kiro-impl.yaml", workflow);
...
test("validator rejects wave collection reverting to planning parent", () => {
  const root = makeCurrentSurfaceFixture();
  const collectPath = join(root, "builtins", "en", "facets", "instructions", "kiro-impl-collect-wave-results.md");
  const collect = readFileSync(collectPath, "utf8").replace("{extends: implement-after-tests}", "{extends: plan}");
  writeFixtureFile(root, "builtins/en/facets/instructions/kiro-impl-collect-wave-results.md", collect);
...
test("validator rejects wave collection that omits COMPLETE part status handling", () => {
  const root = makeCurrentSurfaceFixture();
  const collectPath = join(root, "builtins", "ja", "facets", "instructions", "kiro-impl-collect-wave-results.md");
  const collect = readFileSync(collectPath, "utf8").replaceAll("COMPLETE", "DONE");
  writeFixtureFile(root, "builtins/ja/facets/instructions/kiro-impl-collect-wave-results.md", collect);
...
test("validator rejects wave execution that can invent task branch names", () => {
  const root = makeCurrentSurfaceFixture();
  const executePath = join(root, "builtins", "en", "facets", "instructions", "kiro-impl-execute-task-wave.md");
  const execute = readFileSync(executePath, "utf8").replace("Do not invent wave-id-derived branch names", "Use clear branch names");
  writeFixtureFile(root, "builtins/en/facets/instructions/kiro-impl-execute-task-wave.md", execute);
...
test("validator rejects implementation AI quality gate without local ai-review wiring", () => {
  const root = makeCurrentSurfaceFixture();
  const workflowPath = join(root, "builtins", "ja", "workflows", "kiro-ai-quality-gate.yaml");
  const workflow = readFileSync(workflowPath, "utf8").replace(
    "  ai-antipattern-review: ../facets/instructions/ai-review.md\n",
    "",
  );
  writeFixtureFile(root, "builtins/ja/workflows/kiro-ai-quality-gate.yaml", workflow);
...
test("validator rejects missing AI quality gate workflow", () => {
  const root = makeCurrentSurfaceFixture();
  rmSync(join(root, "builtins", "ja", "workflows", "kiro-ai-quality-gate.yaml"));
...
test("validator rejects missing AI gate evidence hooks in review adapter", () => {
  const root = makeCurrentSurfaceFixture();
  const reviewPath = join(root, "builtins", "en", "facets", "instructions", "kiro-review-task.md");
  const review = readFileSync(reviewPath, "utf8").replaceAll("kiro-ai-antipattern-review.md", "missing-ai-review.md");
  writeFixtureFile(root, "builtins/en/facets/instructions/kiro-review-task.md", review);
...
test("validator rejects adapters that require AI antipattern fix reports unconditionally", () => {
  ...
  writeFixtureFile(root, "builtins/en/facets/instructions/kiro-review-task.md", review);
  const verifyPath = join(root, "builtins", "en", "facets", "instructions", "kiro-verify-task-completion.md");
  const verify = readFileSync(verifyPath, "utf8")
    ...
    .replace("stale or cross-run fix evidence, ", "");
  writeFixtureFile(root, "builtins/en/facets/instructions/kiro-verify-task-completion.md", verify);
...
test("validator rejects verify adapters that ignore AI antipattern fix statuses", () => {
  const root = makeCurrentSurfaceFixture();
  const verifyPath = join(root, "builtins", "ja", "facets", "instructions", "kiro-verify-task-completion.md");
  const verify = readFileSync(verifyPath, "utf8")
    .replace("`STATUS NEED_REPLAN`、`STATUS BLOCKED`、", "")
    .replace("finding-level evidence がない `STATUS NO_FIX_NEEDED`", "weak no-fix evidence");
  writeFixtureFile(root, "builtins/ja/facets/instructions/kiro-verify-task-completion.md", verify);
...
test("validator rejects debug adapters that ignore reviewer child reports", () => {
  const root = makeCurrentSurfaceFixture();
  const debugPath = join(root, "builtins", "en", "facets", "instructions", "kiro-debug-task.md");
  ...
  writeFixtureFile(root, "builtins/en/facets/instructions/kiro-debug-task.md", debug);
...
test("validator rejects debug adapters that ignore implementation result report", () => {
  const root = makeCurrentSurfaceFixture();
  const debugPath = join(root, "builtins", "ja", "facets", "instructions", "kiro-debug-task.md");
  const debug = readFileSync(debugPath, "utf8").replaceAll(
    "kiro-task-implementation-result.md",
    "missing-implementation-result.md",
  );
  writeFixtureFile(root, "builtins/ja/facets/instructions/kiro-debug-task.md", debug);
...
test("validator rejects progress adapter reading AI gate reports directly", () => {
  const root = makeCurrentSurfaceFixture();
  const progressPath = join(root, "builtins", "en", "facets", "instructions", "kiro-impl-update-progress.md");
  const progress = `${readFileSync(progressPath, "utf8")}\nRead kiro-ai-antipattern-review.md before updating progress.\n`;
  writeFixtureFile(root, "builtins/en/facets/instructions/kiro-impl-update-progress.md", progress);
...
test("validator rejects implementation result contract without wave dispatch fields", () => {
  const root = makeCurrentSurfaceFixture();
  const contractPath = join(root, "builtins", "en", "facets", "output-contracts", "kiro-implementation-result.md");
  const contract = readFileSync(contractPath, "utf8").replaceAll("dispatch_mode", "execution_mode");
  writeFixtureFile(root, "builtins/en/facets/output-contracts/kiro-implementation-result.md", contract);
...
test("validator rejects progress policy drift that loses selected task guard", () => {
  const root = makeCurrentSurfaceFixture();
  const policyPath = join(root, "builtins", "ja", "facets", "policies", "kiro-impl-task-progress.md");
  const policy = readFileSync(policyPath, "utf8").replaceAll("selected task", "target task");
  writeFixtureFile(root, "builtins/ja/facets/policies/kiro-impl-task-progress.md", policy);
