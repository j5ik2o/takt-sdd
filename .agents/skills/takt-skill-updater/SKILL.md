...
`references/takt` は source of truth の抽象表記とみなす。`takt-sdd` リポジトリ実体では `references/okite-ai/references/takt` を読む。
...
# root の references/takt が存在しない checkout では、更新元を明示する
TAKT_SRC=/path/to/takt scripts/sync-takt-references.sh --dry-run
TAKT_SRC=/path/to/takt scripts/sync-takt-references.sh
```
...
| takt-task-builder | TaskRecord schema 自体の新規変更はなし。既存の `managed_pr` / `resume_point` 記述を維持し、reference docs を v0.43.0 に同期 |
| takt-workflow-builder | `frontend-maintenance` ワークフロー、peer review への `coding-review` 追加、`quality_gates.type: command` を反映 |
| takt-facet-builder | `coding-reviewer`、`existing-system-respect`、maintenance 系 instruction、`existing-system` knowledge、`coding-review` / `maintenance-scope` output contract をビルトイン例へ追加 |
| takt-analyzer | command quality gate の静的分析観点を追加 |
| takt-optimizer | command quality gate の整理観点を追加 |
...
| takt-task-builder | `managed_pr` 制約、`source: pr_review` と `pr_number` の関係、`resume_point` 構造を `references/task-schema.md` に反映 |
| takt-workflow-builder | `default-draft` / `default-high` / `default-mini` / `default-peer-review` / `draft` / `peer-review` / `auto-improvement-loop` / `takt-default-refresh-*` / CQRS review 系のビルトイン追加を反映。`provider_options.<provider>.effort` の判断軸を追記 |
| takt-facet-builder | templates は廃止ではなく補助参照である点に修正。`ai-antipattern-*` / research / terraform / character persona など v0.42.0 時点のビルトイン一覧へ更新 |
| takt-analyzer | NDJSON レコード名を `workflow_*` に更新し、ログ型参照を `src/shared/utils/types.ts` に修正。`step_start.providerOptions*` を診断観点へ追加 |
| takt-optimizer | `前提 takt バージョン: v0.42.0` に更新 |
| takt-skill-updater | sync スクリプト不在時の `rsync` ベース同期手順を追記 |
