...
- **言語**: インストーラーのソースは TypeScript 6 系、リポジトリの検証スクリプトは JavaScript の ES モジュール。
- **ランタイム**: 開発環境、CI、パッケージ実行は Node.js 24 系に揃える。`@types/node` も 24 系を使用する。
...
Node.js、`@types/node`、TypeScript は `renovate.json` の `allowedVersions` で上記のメジャーに制限する。同じメジャー内のマイナー・パッチ更新は継続し、メジャー更新は明示的な互換性確認後に設定を変更する。
