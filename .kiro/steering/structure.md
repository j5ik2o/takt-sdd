...
takt-sdd は、配布対象のワークフロー資産を、インストーラーのコードや検証コードから分離します。TAKT ワークフローの同梱資産は `builtins/` 配下、project-local override・ユーザー所有 config・実行状態は `.takt/` 配下、インストーラーの挙動は `installer/` 配下、リポジトリレベルの検証は `scripts/` および `tests/` 配下に置きます。
...
- `builtins/en` と `builtins/ja` の同梱資産は、両方が存在する場合は言語ペアの構造を保つこと。
- `.takt/` は project-local override、`eject` 出力、ユーザー所有 config、実行状態の置き場として扱うこと。
