...
この archived spec の過去本文には、旧 `cc-sdd:*` を fail-fast migration shim へ置き換える記述が残っています。現時点の方針ではその記述を採用しません。`cc-sdd:*` は維持し、非推奨として案内します。`cc-sdd:*` を `kiro:*` の透過 alias にせず、fail-fast shim にも置き換えません。新規 `kiro:*` surface は Kiro skill を source of truth とする TAKT workflow へ接続し、公開前に作成された unreleased `kiro-*` workflow/facet は互換維持対象にしません。
...
- `cc-sdd:*` を削除または fail-fast shim へ切り替える decision
...
- 旧 `cc-sdd:*` は透過 alias にせず、fail-fast shim にも置き換えません。既存 workflow として維持し、documentation で非推奨と `kiro:*` 移行先を案内します。
