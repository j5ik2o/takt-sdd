...
- install completion path の順序（asset sync → package.json 更新 → OpenSpec init → cc-sdd init → manifest 書き込み）の変更。
...
- `installer/src/install.ts` — cc-sdd 起動の中核。以下を追加/変更する（各々 1 責務）:
  ...
  - completion path（OpenSpec init / `removeLegacyOpsxScript` 後＝L634 の後、manifest 書き込み L636 の前）に cc-sdd 起動を **無条件**（非 dry-run 時のみ到達）で追加:
    ```ts
    try {
...
- cc-sdd 起動は OpenSpec init と `removeLegacyOpsxScript` の後、manifest 書き込みの前に置く。これにより asset sync・package.json 更新（Req 2.4）と OpenSpec 結果を破壊せず、失敗時は manifest 書き込み・完了表示へ進まない（Req 5.3）。
...
| 2.3 | OpenSpec と同じ completion path（manifest 前） | install() 配置 | call after OpenSpec, before manifest | 起動配置 |
...
| 5.3 | 非ゼロ終了・manifest へ進まない | errorExit 位置（manifest 前） | `process.exit(1)` | 失敗分岐 |
...
- Integration: install() の completion path（OpenSpec init 後、manifest 書き込み前）に配置。
