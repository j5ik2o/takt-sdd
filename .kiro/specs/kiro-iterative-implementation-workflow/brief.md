...
OpenSpec change では、`kiro-impl` を planning、one-task execution、internal review、debug、completion verification、checkbox update の gate を持つ TAKT workflow として実装する方針です。`kiro-review`、`kiro-debug`、`kiro-verify-completion` は `kiro-impl.yaml` 内の internal adapter step として扱います。
...
- internal review/debug/verify adapter step
