...
- [ ] 1. spec generation workflow の validation harness を追加する
  ...
  - 完了時点で missing workflow/facet と lifecycle drift が validation finding として確認できる。
  - _Requirements: 6.1, 6.2, 6.3, 6.4, 6.5_
  - _Boundary:_ SpecGenerationValidationHarness
  - _Depends:_ none

- [ ] 2. spec generation 共通 policy と result contract を追加する
...
- [ ] 3. `kiro-spec-init` workflow と instruction facet を追加する
...
- [ ] 4. requirements 生成 policy と `kiro-spec-requirements` workflow を追加する
...
- [ ] 5. design/research 生成 policy と `kiro-spec-design` workflow を追加する
...
- [ ] 6. task annotation policy と `kiro-spec-tasks` workflow を追加する
...
- [ ] 7. quick path の sanity review contract を追加する
...
- [ ] 8. `kiro-spec-quick` workflow を standalone workflow composition として追加する
  - automatic mode では init、requirements、design、tasks を連続実行し、interactive mode では phase 間 approval を要求する。
  ...
  - _Depends:_ 3, 4, 5, 6, 7

- [ ] 9. spec generation workflow の en/ja language pair をそろえる
...
- [ ] 10. lifecycle と artifact contract の fixture 検証を追加する
  ...
  - _Depends:_ 2, 3, 4, 5, 6

- [ ] 11. generation artifact の section と task annotation 検証を追加する
  - requirements の EARS/numeric ID、design の boundary/file structure/traceability、tasks の `_Boundary:_` と `_Depends:_` を検証する。
...
- [ ] 12. validation harness を test command に接続する
...
- [ ] 13. 下流 spec との境界を最終検証する
  - `kiro-discovery-batch-workflows` が standalone generation workflow を呼ぶだけでよいことを確認する。
...
- [ ] 14. spec generation facets の built-in 継承候補を棚卸しする
