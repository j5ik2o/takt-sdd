...
- 既存 `.agents/skills/kiro-validate-design` の design validation protocol と `kiro-validate-design-readiness` adapter
...
- `.takt/{en,ja}/facets/instructions/kiro-spec-requirements-review.md` — `kiro-spec-requirements` の `Review Requirements Draft` section を TAKT の requirements review/repair loop に接続する thin adapter。
- `.takt/{en,ja}/facets/instructions/kiro-spec-tasks-review.md` — `kiro-spec-tasks` の task plan review と task graph sanity review section を TAKT の tasks review/repair loop に接続する thin adapter。
...
- `.takt/{en,ja}/facets/instructions/kiro-validate-design-readiness.md` — upstream status/validation slice の thin adapter を design review gate の `DECISION` routing field として参照する。この spec は adapter の存在と field contract を利用するが、`kiro-validate-design` workflow の full behavior は所有しない。
...
- `RequirementsGenerationWorkflow` — `.takt/{en,ja}/workflows/kiro-spec-requirements.yaml`、`.takt/{en,ja}/facets/instructions/kiro-spec-requirements.md`、`.takt/{en,ja}/facets/instructions/kiro-spec-requirements-review.md`
- `DesignGenerationWorkflow` — `.takt/{en,ja}/workflows/kiro-spec-design.yaml`、`.takt/{en,ja}/facets/instructions/kiro-spec-design.md`、`.takt/{en,ja}/facets/instructions/kiro-validate-design-readiness.md`
- `TasksGenerationWorkflow` — `.takt/{en,ja}/workflows/kiro-spec-tasks.yaml`、`.takt/{en,ja}/facets/instructions/kiro-spec-tasks.md`、`.takt/{en,ja}/facets/instructions/kiro-spec-tasks-review.md`、`.takt/{en,ja}/facets/policies/kiro-spec-task-annotations.md`
...
Kiro skill が dedicated review field を定義する adapter は、その field を workflow rule の primary condition とします。`kiro-validate-design` は `DECISION`、task plan review は `task_plan_review`、task graph sanity は `task_graph_sanity_review`、quick final sanity は `verdict` を直接読みます。phase generation/finalize step の shared result として `validation.verdict` を使うことはできますが、Kiro skill が定義した review result を `validation.verdict` へ翻訳して分岐することはしません。requirements review は参照元 skill section が dedicated uppercase machine field を持たないため、adapter の明示例外として `validation.verdict` を使います。
...
- `SpecGenerationValidationHarness`: Kiro skill adapter review step が dedicated review field を primary rule condition として使い、requirements review の明示例外以外で review result を `validation.verdict` へ翻訳していないことを検証する。対象: 7.1, 7.3, 7.4, 8.4
