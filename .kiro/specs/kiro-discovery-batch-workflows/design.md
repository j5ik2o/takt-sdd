...
- `package.json` — repository-local validation/test script として `validate:kiro-discovery-batch-workflows` と `test:kiro-discovery-batch-workflows` を追加する。public `kiro:*` script surface の ownership は `kiro-workflow-surface` に残し、この spec は discovery/batch 専用の validation wiring だけを扱う。
...
| 4.1 | generation workflow delegation | BatchWorkerDispatcher | Worker dispatch contract | Batch wave |
...
| 5.6 | batch readiness confirmation | BatchRemediationCoordinator, CrossSpecReviewWorkflow | Output contract | Cross-spec review |
...
| 7.1 | discovery skill adapter inheritance | DiscoveryWorkflow, DiscoveryBatchValidationHarness | `extends_skill` frontmatter | Discovery routing |
| 7.2 | batch skill adapter inheritance | BatchWorkerDispatcher, CrossSpecReviewWorkflow, BatchRemediationCoordinator, DiscoveryBatchValidationHarness | `extends_skill` frontmatter | Batch wave, Cross-spec review |
| 7.3 | thin adapter boundary | DiscoveryWorkflow, BatchWorkerDispatcher, CrossSpecReviewWorkflow, DiscoveryBatchValidationHarness | Skill adapter validation | Validation |
| 7.4 | batch is dependency-wave controller | RoadmapDependencyParser, BatchWavePlanner, BatchWorkerDispatcher | Roadmap parser and batch contract | Batch wave |
| 7.5 | skill section and machine field drift | DiscoveryBatchValidationHarness | Validation script | Validation |
| 8.1 | dynamic worker dispatch | BatchWorkerDispatcher, DiscoveryBatchValidationHarness | Dynamic worker result contract | Batch wave |
| 8.2 | worker uses generation phase contract | BatchWorkerDispatcher, DiscoveryBatchValidationHarness | Generation result contract | Batch wave |
| 8.3 | machine-readable review issue routing | CrossSpecReviewWorkflow, BatchRemediationCoordinator | Cross-spec review contract | Cross-spec review |
| 8.4 | remediation loop threshold | BatchRemediationCoordinator, CrossSpecReviewWorkflow, DiscoveryBatchValidationHarness | `loop_monitors.threshold` | Cross-spec review |
| 8.5 | unresolved issue stop condition | BatchRemediationCoordinator | Batch summary contract | Cross-spec review |
| 8.6 | batch readiness finalization | BatchRemediationCoordinator, CrossSpecReviewWorkflow | Batch summary contract | Cross-spec review |
| 9.1 | reject old single-step wrappers | DiscoveryBatchValidationHarness | Workflow shape validation | Validation |
| 9.2 | require discovery/batch thin adapters | DiscoveryWorkflow, BatchWorkerDispatcher, DiscoveryBatchValidationHarness | Skill adapter validation | Validation |
| 9.3 | reject workflow-to-workflow reuse shims | BatchWorkerDispatcher, DiscoveryBatchValidationHarness | Workflow shape validation | Validation |
| 9.4 | reject unused inherited facets | DiscoveryBatchValidationHarness | Workflow/facet graph validation | Validation |

## Components and Interfaces

| Component | Domain/Layer | Intent | Req Coverage | Key Dependencies | Contracts |
|-----------|--------------|--------|--------------|------------------|-----------|
| DiscoveryWorkflow | Workflow | work request を action path へ分類し、必要な artifact を作る | 1.1, 1.2, 1.3, 1.4, 2.4, 7.1, 7.3, 9.2 | Shared contracts P0 | Batch |
| DiscoveryRoutingPolicy | Policy | action path enum と routing stop condition を定義する | 1.1, 1.4, 2.5, 7.1 | Kiro artifact policy P0 | State |
...
| RoadmapDependencyParser | Parser | roadmap の dependency order と awareness-only section を分ける | 2.2, 2.3, 3.1, 3.3, 7.4 | roadmap P0 | Service |
| BatchWavePlanner | Batch planning | pending specs を dependency wave に分類する | 3.2, 3.3, 3.4, 7.4 | RoadmapDependencyParser P0 | Service |
| BatchWorkerDispatcher | Workflow controller | wave ごとに individual spec generation workflow を呼び出す | 3.5, 4.1, 4.2, 4.3, 4.5, 6.3, 7.2, 7.3, 7.4, 8.1, 8.2, 9.2, 9.3 | Spec generation workflows P0 | Batch |
| CrossSpecReviewWorkflow | Review | generated specs の boundary と contract consistency を確認する | 5.1, 5.2, 5.3, 5.4, 5.5, 5.6, 6.4, 7.2, 8.3, 8.4, 8.6 | generated specs P0 | Batch |
| BatchRemediationCoordinator | Controller | worker failure と review issue の next action をまとめる | 4.2, 4.4, 5.4, 5.5, 5.6, 7.2, 8.3, 8.4, 8.5, 8.6 | BatchWorkerDispatcher P0, CrossSpecReviewWorkflow P0 | State |
| DiscoveryBatchValidationHarness | Validation | discovery/batch workflow と parser contract の drift を検出する | 1.5, 6.1, 6.2, 6.3, 6.4, 6.5, 6.6, 7.1, 7.2, 7.3, 7.5, 8.1, 8.2, 8.4, 9.1, 9.2, 9.3, 9.4 | Node.js 22+ P0 | Service, Batch |
...
- auto-approve mode では shared lifecycle contract に沿って worker-local ready state まで進めるが、その state は batch-level readiness confirmation ではなく cross-spec review input として扱う。
...
- cross-spec review と必要な remediation が完了するまで、worker-local `ready_for_implementation` を downstream implementation-ready signal として確定しない。
...
| Requirements | 4.2, 4.4, 5.4, 5.5, 5.6 |

**Responsibilities & Constraints**

- feature ごとの success、failed、blocked、skipped を summary に集約する。
- local remediation 可能な issue と decomposition return を区別する。
- review/remediation 未完了または blocking issue が残る feature は、spec.json が worker-local ready state でも batch summary 上は implementation-ready として扱わない。
