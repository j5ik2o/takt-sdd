...
- `.takt/{en,ja}/facets/instructions/kiro-collect-validation-evidence.md` — validation workflow 共通の evidence と manual verification の分離規約。
...
- `tests/kiro-status-validation-workflows.test.*` — status/validation workflow の test を既存 test/check command で検出できる場所に追加する。`package.json` の script wiring と `kiro:*` script surface の所有権は本 spec にはない。
...
Validation workflow は artifact state と evidence を分けて扱います。証跡がない項目は manual verification requirement になり、`PASS` verdict の根拠には含めません。
...
| 2.3 | manual verification requirement | KiroGapValidationWorkflow, KiroEvidenceCollector | Evidence contract | Validation Flow |
...
| 5.4 | unverified item の分離 | KiroEvidenceCollector, KiroValidationOutputMapper | Evidence contract | Validation Flow |
...
| KiroEvidenceCollector | Facet instruction | evidence と manual verification requirement を分ける | 2.3, 4.1, 4.3, 5.2, 5.4 | filesystem/codebase evidence P1 | Service |
...
- codebase evidence が不足する場合は manual verification requirement に分離する。
...
- incomplete work、evidence mismatch、manual verification requirement を分けて報告する。
...
- 確認できた evidence と確認できなかった manual verification requirement を混同しない。
...
- Postconditions: observed evidence と manual verification requirement が分離される。
...
- human summary、reason、evidence、manual verification requirement を分ける。
...
- `KiroGapValidationWorkflow` は、requirements missing、requirements present with no evidence、requirements present with observed integration points の fixture で shared validation verdict と manual verification separation を検証する。
...
- `KiroImplValidationWorkflow` は、tasks missing、ready state false、unchecked task、test/build evidence missing、manual verification remaining の fixture で verdict を検証する。
...
- `StatusValidationWorkflowValidationHarness` は、Kiro-specific instruction facet の `extends` frontmatter が shared `BuiltinFacetInheritancePolicy` に従い、親 built-in facet が存在すること、または full custom の理由があることを検証する。

## Integration & Migration Notes

- `kiro-workflow-surface` が `kiro:*` scripts を公開するため、本 spec は package script ownership を持たない。
...
- test/build evidence の取得元は repository ごとに異なるため、初期実装では evidence を「存在確認できた証跡」と「manual verification required」に分けるところまでを contract とし、実行方法の自動推定は避ける。
