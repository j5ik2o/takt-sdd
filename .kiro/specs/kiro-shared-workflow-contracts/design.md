...
- `.takt/en/facets/output-contracts/kiro-review-verdict.md` / `.takt/ja/facets/output-contracts/kiro-review-verdict.md` — `GO`、`NO_GO`、findings、requirement/task references を返す review contract。
...
| `kiro-validation-result` | `verdict` | `PASS`, `FAIL`, `NEEDS_FIX`, `BLOCKED` |
| `kiro-review-verdict` | `verdict` | `GO`, `NO_GO` |
| `kiro-debug-decision` | `decision` | `RETRY_TASK`, `BLOCK_TASK`, `STOP_FOR_HUMAN` |
| `kiro-completion-verification` | `verdict` | `COMPLETE`, `INCOMPLETE`, `BLOCKED` |
...
- 既存 `cc-sdd-*` facets はすぐに削除しない。Kiro shared contract を追加した後、下流 workflow が必要に応じて参照先を `kiro-*` contract へ切り替える。
