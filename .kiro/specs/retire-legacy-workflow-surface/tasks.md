...
- PR #99 レビュー（Cursor Bugbot, Medium）: prefix 非保持の退役専用共有 facet 2 種が cleanup パターン未カバー（要件 5.1 ギャップ）→ `ee82f83` で修正。パターン 4 本目を追加し、cross-check に check 3（RETIRED_EXCLUSIVE_FACETS の両レイアウトカバレッジ）を追加してこのドリフト類型を検証で機械捕捉化。教訓: 名前ベースの forbidden リスト（validator）と prefix ベースの cleanup パターン（installer）の集合は同義ではない — 非 prefix 名は両側で明示列挙が必要
