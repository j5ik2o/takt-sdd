...
Report the result of fixing AI antipattern findings before the normal Kiro review step runs.

## Required Machine Fields

- `STATUS`: one of `FIXED`, `NO_FIX_NEEDED`, `NEED_REPLAN`, or `BLOCKED`.
- `finding_decisions`: list each finding from `kiro-ai-antipattern-review.md` and mark it as fixed, not applicable, requires replan, or blocked.
- `changed_files`: files changed by this fix pass, or `none`.
- `scope_guard`: confirmation that the pass only touched the selected task and did not update task progress.
- `validation_evidence`: tests, commands, or file-level evidence used to support the status.
...
- `NEED_REPLAN`: the issue cannot be fixed safely within the current selected task boundary.
- `BLOCKED`: required information, files, or permissions are unavailable.

The report filename must be `kiro-ai-antipattern-fix.md`.
