...
4. Return `task plan review PASS` and `task graph sanity review PASS` only when both reviews pass.
5. Return `NEEDS_FIXES` when findings are local to the task plan and can be repaired without requirements/design changes.
6. Return `RETURN_TO_DESIGN` when the review exposes a real requirements/design gap or contradiction.
...
- On needs-fixes or return-to-design, include concrete findings and keep `tasks.md` and `spec.json` unchanged.
