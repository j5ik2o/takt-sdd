...
機械的正しさは、implementer の task-local validation output と、skill mode では存在時に親が実行する `.kiro/settings/verify.sh` hook によって証跡化される。coding review は fresh な緑証跡が存在することを確認し、prose だけに依存せず、コード正当性・選択タスク境界・実 diff の判断に集中する。
