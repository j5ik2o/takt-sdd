...
- draft generation または repair step では `tasks.md` の書き込みや `spec.json` の promotion をしない。draft task graph が read-only review step に進める場合は `draft_status: "READY_FOR_REVIEW"` と `review_gate: "PENDING"` を返す。
