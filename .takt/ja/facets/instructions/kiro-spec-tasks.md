...
3. `-y` または `auto-approve` mode が有効な場合は、成功する tasks phase の一部として `approvals.requirements.approved: true` と `approvals.design.approved: true` を明示的に更新する。暗黙の approval として扱わない。
...
7. success metadata を書く前に task plan review を実行する。review は executable task size、observable completion、numeric requirements、hidden prerequisites、requirement coverage を確認する。
8. success metadata を書く前に task graph sanity review を実行する。review は Boundary annotation coverage、Depends annotation coverage、dependency graph validity、boundary overlap、`(P)` marker validity を確認する。
...
11. 両方の review が通過した場合だけ `.kiro/specs/<feature>/tasks.md` を書く。
12. 同じ成功 result で `.kiro/specs/<feature>/spec.json` を更新する。
...
- draft generation または repair step では read-only review step が読む draft artifact として `tasks.md` を書く、または更新する。ただし `spec.json` を `tasks-generated` へ promotion してはならない。draft task graph が review に進める場合は `draft_status: "READY_FOR_REVIEW"` と `review_gate: "PENDING"` を返す。
- 成功時は `phase: "tasks"`、`validation.verdict: "PASS"`、`featureName`、`updatedFiles` に `tasks.md` と `spec.json` を含める。
- task plan review と task graph sanity review の通過後の finalize step では、artifact update とともに `draft_status: "WRITTEN"` と `review_gate: "PASSED"` を返す。
