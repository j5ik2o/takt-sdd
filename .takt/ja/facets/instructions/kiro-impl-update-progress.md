...
1. ステージ対象を収集する。implementation report の `changed_files` を起点とし、今回の ai-quality-gate サブワークフロー実行で AI アンチパターン是正レポートが生成されていれば、そのレポートの `changed_files` も追加する。選択中 spec の task file `.kiro/specs/<feature>/tasks.md` は常に含める。収集したファイルを和集合としてステージ: `git add <implementation_changed_files> [<antipattern_fix_changed_files>] .kiro/specs/<feature>/tasks.md`（`git add -A` は絶対に使わない）
