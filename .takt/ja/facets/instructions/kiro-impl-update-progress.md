...
1. ステージ対象を収集する。implementation report の `changed_files` を起点とし、今回の実行で AI アンチパターン是正レポート（ai-quality-gate サブワークフローの fix ステップが書き出すレポート）が生成されていれば、そのレポートの `changed_files` も追加する。`tasks.md` は常に含める。収集したファイルを和集合としてステージ: `git add <implementation_changed_files> [<antipattern_fix_changed_files>] tasks.md`（`git add -A` は絶対に使わない）
