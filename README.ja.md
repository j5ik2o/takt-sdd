...
`takt-sdd` は npm グローバルパッケージとして提供されており、repo ローカルの npm scripts を使わずに、任意のプロジェクトから `kiro-*` / `opsx-*` ワークフローを実行できる。
...
| `opsx-propose` | 変更を作成し、全アーティファクトを生成 |
| `opsx-apply` | 変更のタスクを実装 |
| `opsx-archive` | 完了した変更をアーカイブ |
| `opsx-explore` | 対話的な探索と思考（pipeline モード — 後述の注意を参照） |
...
| `--dry-run` | ファイルを書き込まず、OpenSpec init も cc-sdd init も実行せずに変更予定を表示 |

`--tag` は global CLI では**サポートされていない**。インストール済みパッケージに同梱された資産が常に使用される。

### legacy `cc-sdd-*` workflow の拒否

global CLI は `cc-sdd-*` workflow を直接形式・`run` 形式ともに拒否する：

```bash
takt-sdd cc-sdd-full        # エラー: legacy cc-sdd-* workflows are not supported
takt-sdd run cc-sdd-full    # エラー: 同様に拒否
```

`cc-sdd:*` 互換機能はプロジェクトの npm scripts を通じて引き続き利用可能（プロジェクトが管理する別経路）。

### `opsx-explore` の実行モード差

global CLI 経由（`takt-sdd opsx-explore`）では、全 supported workflow と同様に **pipeline モード**（`--pipeline --skip-git`）で実行される。  
プロジェクトの npm script 経由（`npm run opsx:explore`）では `--pipeline` **なし**で実行される（対話モード）。対話的な探索を利用する場合は npm script を使うこと。
